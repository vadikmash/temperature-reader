 ...
 *    by Vadzim Mashnitski (vadikmash)
 *    (refactor, array output as JSON)
 */
 ...
 Serial.print("[");
  ...
  for(int y = 0; y < 4; ++y){ // go through all the rows
    ...
    for(int x = 0; x < 16; x++){ // go through all the columns
      float tempAtXY= sensor.getTemperature(y + x * 4); // extract the temperature at position (x, y)
...
// this loop gives us approximately 8 temperature arrays per second
void loop() {  
